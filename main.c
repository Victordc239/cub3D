...
/*   Updated: 2025/12/01 11:32:01 by victor           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* void	update_player(t_game *g, double delta, double move_speed, double rot_speed)
{
	double	nx;
	double	ny;
	double	old_dir_x;
...
void	render_frame(t_game *g)
{
	...
	while (x < g->screen_w)
	{
		...
		while (!hit)
		{
			...
			if (map_y < 0 || map_y >= g->map_height
				|| map_x < 0 || map_x >= g->map_width)
			{
				hit = 1;
				break ;
			}
